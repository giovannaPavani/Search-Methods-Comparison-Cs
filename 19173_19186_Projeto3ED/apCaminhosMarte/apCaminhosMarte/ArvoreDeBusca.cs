...
namespace apCaminhosMarte
{
    class ArvoreDeBusca<Dado>  // : IComparable<NoArvore<Dado>> 
                                where Dado : IComparable<Dado>, IRegistro, ICloneable, new()
    {
        NoArvore<Dado> raiz,
        ...
        public void InserirBalanceado(Dado dadoLido)
        {
            InserirBalanceado(dadoLido, ref raiz);
        }

        private NoArvore<Dado> InserirBalanceado(Dado item, ref NoArvore<Dado> noAtual)
        {
            // insere as informações na ávores recursivamente, balanceando-a de acordo com o CompareTo da classe Dado
            if (noAtual == null)
                noAtual = new NoArvore<Dado>(item);
            else
            {
                if (item.CompareTo(noAtual.Info) < 0) // se o item a ser add é menor que o atual
                {
                    // ele é inserido à esquerda
                    NoArvore<Dado> atualEsq = (NoArvore<Dado>)noAtual.Esq;
                    noAtual.Esq = InserirBalanceado(item, ref atualEsq);
                ...
                if (item.CompareTo(noAtual.Info) > 0) // se o item a ser add é maior que o atual
                {
                    // ele é inserido à direita
                    NoArvore<Dado> atualDir = (NoArvore<Dado>)noAtual.Dir;
                    noAtual.Dir = InserirBalanceado(item, ref atualDir);
        ...
        {
            // faz-se a troca de lugar entre um nó e seu filho esquerdo
        ...
        {
            // faz-se a troca de lugar entre um nó e seu filho direito
